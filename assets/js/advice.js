...
export function buildAdvice(result, rawSituation = {}, today = new Date()) {
  ...
  if (result.input.hasSpouse && taxed) {
    push({
      ...
      body:
        ...
        '다만 배우자에게 많이 몰아주면 나중에 2차 상속(배우자 사망) 때 세부담이 커지므로 함께 따져봐야 합니다.',
      cta: true,
    });
  }
  ...
  if (s.hasBusinessAsset) {
    push({
      ...
      level: 'critical',
      priority: 87,
      title: '가업상속공제는 최대 600억원까지 가능하지만 이 계산기에는 반영되지 않았습니다',
      body:
        '가업상속공제나 영농상속공제는 요건(피상속인 경영기간, 상속인 종사요건, 사후관리 등)이 까다로운 대신 ' +
        '공제 규모가 매우 큽니다. 해당된다면 실제 세액은 여기 표시된 금액보다 훨씬 적을 수 있습니다.',
      cta: true,
    });
  }
